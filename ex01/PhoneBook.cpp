...
void	PhoneBook::printTable(void)
{
	...
	for (int idx = 0; idx < 8; idx++)
	{
		if (!this->list[idx].getName().empty())
		{
			...
			std::cout << std::setw(10) << truncate(this->list[idx].getNickname(), 10) << "|";
			std::cout << std::setw(10) << truncate(this->list[idx].getNumber(), 10) << "|";
			std::cout << std::setw(10) << truncate(this->list[idx].getSecret(), 10) << std::endl;
		}
	}
}
